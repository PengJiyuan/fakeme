...
var checkGlobal = checkName;
var checkEmail = function (opts) {
  if (opts && !isObject(opts)) {
    throw 'param option should be an object';
  }
  if (opts && opts.sex && !~sexs.indexOf(opts.sex)) {
...
function getSex (opts) {
  return sexs$1[opts.lang][random(0, 1)];
}
...
Fake.prototype.name = function name (opts) {
  ...
  var firstName = firstNames.names[random(0, firstNames.count - 1)];
  var lastNames = names$$1[_opts.lang].lastName;
  var lastName = lastNames.names[random(0, lastNames.count - 1)];
  var name;
...
Fake.prototype.id = function id (length) {
    if ( length === void 0 ) length = 10;
...
Fake.prototype.random = function random$1 (list, count) {
    if ( list === void 0 ) list = [];
